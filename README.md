...
- Инспектор: название, план, дедлайны, тип, исполнитель, связи с
  ...
  описание сохраняются сами через паузу набора. Удаление — корзина с
  подтверждением в поповере.
...
- Привязка живёт на «логической» задаче: у серии повторов заметка видна
  на всех вхождениях и автоматически переходит на следующее после
  отметки ✓; в поиске пикера серия — одна строка (последнее вхождение).
  Открепление снимает заметку со всей серии.
...
- Рядом — типы связей задач: у направленных два названия («блокирует» /
  ...
  Сами связи вешаются в инспекторе задачи. Как и заметки, связь живёт на
  логической задаче: у серии повторов видна на всех вхождениях и
  переживает спавн следующего после ✓.
