...
describe("repeat", () => {
  ...
  it("призраки: строго после плановой даты, в границах окна", () => {
    ...
    expect(
      ghostOccurrences(t, "2030-01-07", "2030-01-20", "2030-01-01"),
    ).toEqual(["2030-01-10", "2030-01-14", "2030-01-17"]);
  });
  it("просроченная задача: призраки только строго после сегодня", () => {
    // план — прошлый пн, сегодня чт: пн/вт/ср/чт призраков нет
    const t = task({
      scheduledOn: "2030-01-07",
      repeat: { kind: "weekly", days: [1, 2, 3, 4, 5] },
    });
    expect(
      ghostOccurrences(t, "2030-01-07", "2030-01-14", "2030-01-10"),
    ).toEqual(["2030-01-11", "2030-01-14"]);
  });
  it("призраки: сама плановая дата не дублируется, окно до неё — пусто", () => {
    ...
    expect(
      ghostOccurrences(t, "2030-01-01", "2030-01-07", "2030-01-01"),
    ).toEqual([]);
  });
